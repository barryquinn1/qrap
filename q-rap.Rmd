...
## Why R and Python?
R and Python are by far the leading languages for data analysis in finance industry.  This success is slowly infiltrating academia, although there are some notable holdouts where econometrics is taught using commercial graphical user interfaces(GUI).  Proponents of GUI-based econometrics teaching argue that teaching statistical concepts is less intimidating to beginners when using a point-and-click approach compared to command line approach. The latter also requires teaching programming in tandem with statistics.  

Our experience has shown that such convenience is only possible by removing data analysis from the course content and providing students with tidy, rectangular data.  But for modern financial data science this approach is a disservice to students.  Furthermore, point-and-click approaches require a bespoke student user manual than can run to 40-plus pages provide a considerable learning curve for the novice student which isn’t generalisable to other analytics workflows.

 
  The use of GUI-based econometrics is assumed to be more frictionless for teaching, but the GUI *copy and paste* workflow can actually increase frictions, is more error prone, and hard to debug than command line R and Python syntax[@].  

### Teaching statistics by simulation
A meaningful understand of econometrics requires more than reading and regurgitating textbook material.  The false discovery crisis in finance suggests that applying out-of-the-box econometric models to observational data also fails to provide a deep understanding of the qasm of abstraction of econometric models from financial data [@lopez2020].  
