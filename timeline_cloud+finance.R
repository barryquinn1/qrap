...
c("NASDAQ launched as first electronic communications market",
  "Fischer Black proposes idea of fully electronic exchanges in a landmark paper",
  "Black-Scholes-Merton model for derivative pricing introduced",
  ...
  "SEC order US stock exchanges to be decimalised")->financeMs
...
c("Professor JohnMcCarthy (MIT) suggested computing will be sold as a utility",
  ...
  "Dropbox launch Cloud storage")->cloudMs            
content<-c(financeMs,cloudMs)            
start   = paste0(c("1971","1972","1973","1982","1983","1988","1993","2000","2001","1961","1967","1969","1988","1991","1996","2002","2005","2006","2007"),"-01-01")
...
p<-gg_vistime(timeline_data, optimize_y = FALSE, show_labels = FALSE,
              title="Evolution of quantitative finance and cloud computing: The early years") +
  geom_label_repel(aes(label=timeline_data$event)) +
  dark_theme_dark()
ggsave("img/t1.png",plot = p,height = 9,width = 16,dpi=400)


c("Flash Crash (Markets drop 10% in a matter of minutes)",
...
c("Microsoft launch Azure cloud computing",
...
content<-c(financeMs,cloudMs)
start   = paste0(c("2010","2013","2017","2018","2019","2010"
                   ,"2012","2013","2015","2018","2019"),"-01-01")

i<-length(financeMs)
j=length(cloudMs)

timeline_data<-data.frame(
  event =str_wrap(content,width=10),
  start=start,
  end=start,
  group=c(rep("Finance",i),rep("Cloud computing",j)))

p<-gg_vistime(timeline_data, optimize_y = FALSE, show_labels = FALSE,
              title="Evolution of quantitative finance and cloud computing: Latest decade") +
  geom_label_repel(aes(label=timeline_data$event)) +
  dark_theme_dark()

ggsave("img/t2.png",plot = p,height = 9,width = 16,dpi=400)